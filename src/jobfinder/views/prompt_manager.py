...
def get_selected_data() -> List[Dict]:
    ...
    selected_df = st.session_state.template_data[
        st.session_state.template_data['instruction_id'].isin(st.session_state.selected_instructions)
    ]
...
# Create tabs
tab1, tab2, tab3, tab4 = st.tabs(["📝 Edit Prompt", "📊 Analytics", "📚 History", "🔧 Template Builder"])
    ...
    if st.session_state.prompt_history:
        ...
        if len(history_data) > 1:
            ...
            with col_chart1:
                ...
                st.line_chart(data=[v['word_count'] for v in reversed(st.session_state.prompt_history)])
            
            with col_chart2:
                st.subheader("Character Count Trend")
                st.line_chart(data=[v['char_count'] for v in reversed(st.session_state.prompt_history)])
    else:
        st.info("No prompt versions saved yet. Save a version in the Edit tab to see analytics.")
    ...
    if st.session_state.prompt_history:
        for i, version in enumerate(st.session_state.prompt_history):
            with st.expander(
                ...
                with col_info:
                    st.markdown(f"**Stats:** {version['word_count']} words, {version['char_count']} characters")
...
with tab4:
    ...
    with col_data:
        ...
        with st.expander("🔧 Data Management"):
            ...
            if uploaded_csv is not None:
                try:
                    ...
                    st.session_state.template_data = new_data
        ...
        # Apply filters
        if category_filter:
            df_display = df_display[df_display['category'].isin(category_filter)]
        ...
        if selection_mode == "Individual Selection":
            ...
            selection_df = df_display[['instruction_id', 'category', 'title', 'priority', 'active']].copy()
        ...
        # Show selected count
        st.info(f"📋 Selected {len(st.session_state.selected_instructions)} instructions")
        
        # Quick actions
        quick_col1, quick_col2, quick_col3 = st.columns(3)
        with quick_col1:
            if st.button("🔄 Select All Active", use_container_width=True):
                active_ids = df_display[df_display['active'] == True]['instruction_id'].tolist()
                st.session_state.selected_instructions = active_ids
                st.rerun()
        
        with quick_col2:
            if st.button("⭐ Select High Priority", use_container_width=True):
                high_priority_ids = df_display[df_display['priority'] >= 3]['instruction_id'].tolist()
...
{% endfor %}"""
        ...
        selected_preset = st.selectbox("Choose a preset template:", list(preset_templates.keys()))
